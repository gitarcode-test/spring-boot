...
class OAuth2ResourceServerAutoConfigurationTests {
  ...
  private void assertJwkSetUriJwtDecoderBuilderCustomization(
      AssertableWebApplicationContext context) {
    JwkSetUriJwtDecoderBuilderCustomizer customizer =
        context.getBean("decoderBuilderCustomizer", JwkSetUriJwtDecoderBuilderCustomizer.class);
    JwkSetUriJwtDecoderBuilderCustomizer anotherCustomizer =
        context.getBean(
            "anotherDecoderBuilderCustomizer", JwkSetUriJwtDecoderBuilderCustomizer.class);
        ...
        .run(
            (context) -> {
              ...
              assertThat(jwtDecoder)
                  .extracting("jwtProcessor.jwsKeySelector.jwsAlgs")
    ...
    this.contextRunner
        ...
        .run(
            (context) -> {
              ...
              assertThat(nimbusJwtDecoder)
                  .extracting("jwtProcessor.jwsKeySelector.expectedJWSAlg")
        ...
        .run(
            (context) -> {
              ...
              Supplier<JwtDecoder> jwtDecoderSupplier =
                  (Supplier<JwtDecoder>)
                      ReflectionTestUtils.getField(supplierJwtDecoderBean, "delegate");
  ...
  @Test
  void autoConfigurationShouldConfigureAudienceAndIssuerJwtValidatorIfPropertyProvided()
      throws Exception {
    this.server = new MockWebServer();
    this.server.start();
    String path = "test";
  ...
  void autoConfigurationShouldConfigureCustomValidators() throws Exception {
    ...
    String issuerUri =
        "http://" + this.server.getHostName() + ":" + this.server.getPort() + "/" + path;
    this.contextRunner
        .withPropertyValues("spring.security.oauth2.resourceserver.jwt.issuer-uri=" + issuerUri)
  ...
  void shouldNotConfigureJwtConverterIfNoPropertiesAreSet() {
    this.contextRunner.run(
        (context) -> assertThat(context).doesNotHaveBean(JwtAuthenticationConverter.class));
  }
