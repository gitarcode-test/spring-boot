 ...
 *
 * <p>It delegates work to Flyways {@link Scanner}, and additionally uses {@link
...
class NativeImageResourceProvider implements ResourceProvider {
  ...
  public Collection<LoadableResource> getResources(String prefix, String[] suffixes) {
    ...
    ensureInitialized();
    Predicate<LocatedResource> matchesPrefixAndSuffixes =
        (locatedResource) ->
            StringUtils.startsAndEndsWith(locatedResource.resource.getFilename(), prefix, suffixes);
  ...
  private Resource[] getResources(
      PathMatchingResourcePatternResolver resolver, Location location, Resource root) {
    try {
      return resolver.getResources(root.getURI() + "/*");
    } catch (IOException ex) {
      throw new UncheckedIOException(
          "Failed to list resources for " + location.getDescriptor(), ex);
    }
  }
