...
/**
 * A {@link RouterFunction} factory for an application's welcome page. Supports both static and
 * templated files. If both a static and templated index page are available, the static page is
 * preferred.
 *
 * @author Brian Clozel
 */
final class WelcomePageRouterFunctionFactory {
  ...
  WelcomePageRouterFunctionFactory(
    ...
    this.welcomePageTemplateExists =
  ...
  RouterFunction<ServerResponse> createRouterFunction() {
    if (this.welcomePage != null && "/**".equals(this.staticPathPattern)) {
      return RouterFunctions.route(
          ...
          (req) ->
              ServerResponse.ok().contentType(MediaType.TEXT_HTML).bodyValue(this.welcomePage));
    } else if (this.welcomePageTemplateExists) {
      return RouterFunctions.route(
          GET("/").and(accept(MediaType.TEXT_HTML)), (req) -> ServerResponse.ok().render("index"));
    }
    return null;
  }
}
