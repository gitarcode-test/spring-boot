...
/**
 * A {@link FailureAnalyzer} that performs analysis of failures caused by an {@link
...
class InvalidConfigurationPropertyValueFailureAnalyzer
  ...
  protected FailureAnalysis analyze(
      Throwable rootFailure, InvalidConfigurationPropertyValueException cause) {
  ...
  private void appendDetails(
    ...
    message
        ...
        .append(mainDescriptor.getValue())
  ...
  private void appendReason(
      StringBuilder message, InvalidConfigurationPropertyValueException cause) {
  ...
  private void appendAdditionalProperties(StringBuilder message, List<Descriptor> descriptors) {
    ...
    if (!others.isEmpty()) {
      message.append(
          String.format(
              "%n%nAdditionally, this property is also set in the following property %s:%n%n",
