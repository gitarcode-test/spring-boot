...
/**
 * A {@link FailureAnalyzer} that performs analysis of failures caused by a {@link
...
class MutuallyExclusiveConfigurationPropertiesFailureAnalyzer
  ...
  protected FailureAnalysis analyze(
      Throwable rootFailure, MutuallyExclusiveConfigurationPropertiesException cause) {
    ...
    return new FailureAnalysis(
        ...
        "Update your configuration so that only one of the mutually exclusive properties is"
            + " configured.",
        cause);
  }
  ...
  private void appendDetails(
    ...
    message.append(
        String.format(
            "However, more than one of those properties has been configured at the same"
                + " time:%n%n"));
    Set<String> configuredDescriptions =
        sortedStrings(
            descriptors,
            (descriptor) ->
                String.format(
                    ...
                    (descriptor.origin != null)
                        ? " (originating from '" + descriptor.origin + "')"
                        : ""));
