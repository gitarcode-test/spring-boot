...
/**
 * {@link ConfigurationPropertySource} backed by an {@link EnumerablePropertySource}. Extends {@link
 * SpringConfigurationPropertySource} with full "relaxed" mapping support. In order to use this
 * adapter the underlying {@link PropertySource} must be fully enumerable. A security restricted
 * {@link SystemEnvironmentPropertySource} cannot be adapted.
 *
 * @author Phillip Webb
  ...
  private ConfigurationPropertyName[] getConfigurationPropertyNames() {
    ...
    if (configurationPropertyNames == null) {
      configurationPropertyNames =
          getMappings().getConfigurationPropertyNames(getPropertySource().getPropertyNames());
  ...
  private static class Mappings {
    ...
    private void updateMappings(String[] propertyNames) {
      ...
      Map<String, ConfigurationPropertyName> reverseMappings =
          cloneOrCreate(this.reverseMappings, size);
      ...
      this.configurationPropertyNames =
          this.immutable
              ? reverseMappings.values().toArray(new ConfigurationPropertyName[0])
              : null;
    }
