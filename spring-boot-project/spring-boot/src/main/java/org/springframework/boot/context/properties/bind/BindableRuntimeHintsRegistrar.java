...
/**
 * {@link RuntimeHintsRegistrar} that can be used to register {@link ReflectionHints} for {@link
 * Bindable} types, discovering any nested type it may expose through a property.
 *
 * <p>This class can be used as a base-class, or instantiated using the {@code forTypes} and {@code
...
public class BindableRuntimeHintsRegistrar implements RuntimeHintsRegistrar {
  ...
  public static BindableRuntimeHintsRegistrar forBindables(Iterable<Bindable<?>> bindables) {
    ...
    return forBindables(
        StreamSupport.stream(bindables.spliterator(), false).toArray(Bindable[]::new));
  }
  ...
  private static final class Processor {
    ...
    private Processor(Bindable<?> bindable, boolean nestedType, Set<Class<?>> seen) {
      ...
      this.bindConstructor =
          (bindable.getBindMethod() != BindMethod.JAVA_BEAN)
              ? BindConstructorProvider.DEFAULT.getBindConstructor(
                  bindable.getType().resolve(), nestedType)
              : null;
    ...
    private void handleValueObjectProperties(ReflectionHints hints) {
      for (int i = 0; i < this.bindConstructor.getParameterCount(); i++) {
        ...
        ResolvableType propertyType =
            ResolvableType.forConstructorParameter(this.bindConstructor, i);
    ...
    private void handleProperty(
        ReflectionHints hints, String propertyName, ResolvableType propertyType) {
    ...
    /**
     * Specify whether the specified property refer to a nested type. A nested type represents a
     * sub-namespace that need to be fully resolved. Nested types are either inner classes or
     * annotated with {@link NestedConfigurationProperty}.
     *
    ...
    private static boolean isNested(Class<?> type, Class<?> candidate) {
      ...
      return (candidate.getDeclaringClass() != null
          && isNested(type, candidate.getDeclaringClass()));
    }
  ...
  private static final class KotlinDelegate {
    ...
    static void handleConstructor(ReflectionHints hints, Constructor<?> constructor) {
      ...
      if (kClass.isData()) {
        hints.registerType(
            constructor.getDeclaringClass(), MemberCategory.INVOKE_DECLARED_CONSTRUCTORS);
      } else {
