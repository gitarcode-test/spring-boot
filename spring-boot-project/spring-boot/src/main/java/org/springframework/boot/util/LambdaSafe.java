...
/**
 * Utility that can be used to invoke lambdas in a safe way. Primarily designed to help support
 * generically typed callbacks where {@link ClassCastException class cast exceptions} need to be
 * dealt with due to class erasure.
 *
 * @author Phillip Webb
 * @since 2.0.0
 */
public final class LambdaSafe {
  ...
  /**
   * Start a call to a single callback instance, dealing with common generic type concerns and
   * exceptions.
   *
  ...
  protected abstract static class LambdaSafeCallback<
      C, A, SELF extends LambdaSafeCallback<C, A, SELF>> {
    ...
    /**
     * Use a specific filter to determine when a callback should apply. If no explicit filter is set
     * filter will be attempted using the generic type on the callback type.
     *
    ...
    protected final <R> InvocationResult<R> invoke(C callbackInstance, Supplier<R> supplier) {
      if (this.filter.match(
          this.callbackType, callbackInstance, this.argument, this.additionalArguments)) {
        try {
    ...
    private boolean startsWithArgumentClassName(String message, Object argument) {
      ...
      if (moduleSeparatorIndex != -1
          && message.startsWith(argumentType.getName(), moduleSeparatorIndex + 1)) {
        return true;
      }
    ...
    private void logNonMatchingType(C callback, ClassCastException ex) {
      if (this.logger.isDebugEnabled()) {
        ...
        String expectedTypeName =
            (expectedType != null) ? ClassUtils.getShortName(expectedType) + " type" : "type";
        String message =
            "Non-matching "
                + expectedTypeName
                + " for callback "
                + ClassUtils.getShortName(this.callbackType)
                + ": "
                + callback;
  ...
  public static final class Callback<C, A> extends LambdaSafeCallback<C, A, Callback<C, A>> {
     ...
     * @return the result of the invocation (may be {@link InvocationResult#noResult} if the
  ...
  public static final class Callbacks<C, A> extends LambdaSafeCallback<C, A, Callbacks<C, A>> {
     ...
     * @param invoker the invoker used to invoke the callback
     * @param <R> the result type
     * @return the results of the invocation (may be an empty stream if no callbacks could be
     *     called)
     */
  ...
  /**
   * {@link Filter} that matches when the callback has a single generic and primary argument is an
   * instance of it.
   */
  ...
  /**
   * The result of a callback which may be a value, {@code null} or absent entirely if the callback
   * wasn't suitable. Similar in design to {@link Optional} but allows for {@code null} as a valid
   * value.
   *
   * @param <R> the result type
   */
