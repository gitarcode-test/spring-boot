...
/**
 * A {@link ConfigurationPropertySource} backed by a {@link Map} and using standard name mapping
 * rules.
 *
 * @author Phillip Webb
...
public class MapConfigurationPropertySource implements IterableConfigurationPropertySource {
  ...
  /**
   * Create a new {@link MapConfigurationPropertySource} instance with entries copies from the
   * specified map.
   *
  ...
  public MapConfigurationPropertySource(Map<?, ?> map) {
    ...
    this.delegate =
        new SpringIterableConfigurationPropertySource(mapPropertySource, DEFAULT_MAPPERS);
    putAll(map);
  }
