...
public class StandardConfigDataLocationResolver
  ...
  private static final Pattern EXTENSION_HINT_PATTERN =
      Pattern.compile("^(.*)\\[(\\.\\w+)](?!\\[)$");
   ...
   */
  public StandardConfigDataLocationResolver(
      DeferredLogFactory logFactory, Binder binder, ResourceLoader resourceLoader) {
  ...
  private String[] getConfigNames(Binder binder) {
    String[] configNames =
        binder.bind(CONFIG_NAME_PROPERTY, String[].class).orElse(DEFAULT_CONFIG_NAMES);
  ...
  private Set<StandardConfigDataReference> getReferences(
    ...
    try {
      ...
      throw new IllegalStateException(
          "Unable to load config data from '" + configDataLocation + "'", ex);
    }
  }
  ...
  private String getResourceLocation(
    ...
    boolean isAbsolute =
        resourceLocation.startsWith("/") || URL_PREFIX.matcher(resourceLocation).matches();
    if (isAbsolute) {
    ...
    if (parent instanceof StandardConfigDataResource resource) {
      ...
      String parentDirectory =
          parentResourceLocation.substring(0, parentResourceLocation.lastIndexOf("/") + 1);
  ...
  private Deque<StandardConfigDataReference> getReferencesForConfigName(
      String name, ConfigDataLocation configDataLocation, String directory, String profile) {
    ...
    for (PropertySourceLoader propertySourceLoader : this.propertySourceLoaders) {
      for (String extension : propertySourceLoader.getFileExtensions()) {
        StandardConfigDataReference reference =
            new StandardConfigDataReference(
                configDataLocation,
                directory,
                directory + name,
                profile,
  ...
  private Set<StandardConfigDataReference> getReferencesForFile(
      ...
      if (extension != null) {
        ...
        StandardConfigDataReference reference =
            new StandardConfigDataReference(
                ...
                root,
                profile,
                (!extensionHintLocation) ? extension : null,
                propertySourceLoader);
    ...
    if (configDataLocation.hasPrefix(PREFIX)
      ...
      throw new IllegalStateException(
          "File extension is not known to any PropertySourceLoader. If the location is meant to"
              + " reference a directory, it must end in '/' or File.separator");
    }
    throw new IllegalStateException(
        "Incorrect ConfigDataLocationResolver chosen or file extension is not known to any"
            + " PropertySourceLoader. If the location is meant to reference a directory, it must"
            + " end in '/' or File.separator. The location is being resolved using the"
            + " StandardConfigDataLocationResolver, check the location prefix if a different"
  ...
  private Set<StandardConfigDataResource> resolveEmptyDirectories(
      StandardConfigDataReference reference) {
  ...
  private Set<StandardConfigDataResource> resolveNonPatternEmptyDirectories(
      StandardConfigDataReference reference) {
  ...
  private Set<StandardConfigDataResource> resolvePatternEmptyDirectories(
      StandardConfigDataReference reference) {
    Resource[] subdirectories =
        this.resourceLoader.getResources(reference.getDirectory(), ResourceType.DIRECTORY);
    ConfigDataLocation location = reference.getConfigDataLocation();
    if (!location.isOptional() && ObjectUtils.isEmpty(subdirectories)) {
      String message =
          String.format("Config data location '%s' contains no subdirectories", location);
  ...
  private List<StandardConfigDataResource> resolveNonPattern(
      StandardConfigDataReference reference) {
  ...
  private List<StandardConfigDataResource> resolvePattern(StandardConfigDataReference reference) {
    ...
    for (Resource resource :
        this.resourceLoader.getResources(reference.getResourceLocation(), ResourceType.FILE)) {
      if (!resource.exists() && reference.isSkippable()) {
        logSkippingResource(reference);
      } else {
