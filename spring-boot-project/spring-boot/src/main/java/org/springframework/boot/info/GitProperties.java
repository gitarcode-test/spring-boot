...
public class GitProperties extends InfoProperties {
  ...
  static final Set<Coercer> coercers =
      Set.of(
          Coercer.milliseconds(),
          Coercer.dateTimePattern("yyyy-MM-dd'T'HH:mm:ssXXX"),
          Coercer.dateTimePattern("yyyy-MM-dd'T'HH:mm:ssZ"));
   ...
   *
   * <p>If the original value could not be parsed properly, it is still available with the {@code
   * commit.time} key.
   *
  ...
  private record Coercer(
      Function<String, Long> action, Predicate<RuntimeException> ignoredExceptions) {
    ...
    static Coercer milliseconds() {
      return new Coercer(
          (value) -> Long.parseLong(value) * 1000, NumberFormatException.class::isInstance);
    }

    static Coercer dateTimePattern(String pattern) {
      return new Coercer(
          (value) ->
              DateTimeFormatter.ofPattern(pattern).parse(value, Instant::from).toEpochMilli(),
