...
public final class ConfigurationPropertySources {
  ...
  /**
   * Create a new {@link PropertyResolver} that resolves property values against an underlying set
   * of {@link PropertySources}. Provides an {@link ConfigurationPropertySource} aware and optimized
   * alternative to {@link PropertySourcesPropertyResolver}.
   *
  ...
  /**
   * Determines if the specific {@link PropertySource} is the {@link ConfigurationPropertySource}
   * that was {@link #attach(Environment) attached} to the {@link Environment}.
   *
   ...
   * Adapts each {@link PropertySource} managed by the environment to a {@link
   * ConfigurationPropertySource} and allows classic {@link PropertySourcesPropertyResolver} calls
   * to resolve using {@link ConfigurationPropertyName configuration property names}.
   *
   * <p>The attached resolver will dynamically track any additions or removals from the underlying
   * {@link Environment} property sources.
   *
   * @param environment the source environment (must be an instance of {@link
  ...
  private static boolean isUsingSources(
      PropertySource<?> attached, MutablePropertySources sources) {
  ...
  /**
   * Return a set of {@link ConfigurationPropertySource} instances that have previously been {@link
   * #attach(Environment) attached} to the {@link Environment}.
   *
   * @param environment the source environment (must be an instance of {@link
   *     ConfigurableEnvironment})
   * @return an iterable set of configuration property sources
   * @throws IllegalStateException if not configuration property sources have been attached
   */
   ...
   * @return an {@link Iterable} containing a single newly adapted {@link
   ...
   * from the given Spring {@link PropertySource PropertySources}.
   *
   * <p>This method will flatten any nested property sources and will filter all {@link
   * StubPropertySource stub property sources}. Updates to the underlying source, identified by
   * changes in the sources returned by its iterator, will be automatically tracked. The underlying
   * source should be thread safe, for example a {@link MutablePropertySources}
   *
   * @param sources the Spring property sources to adapt
   * @return an {@link Iterable} containing newly adapted {@link SpringConfigurationPropertySource}
   *     instances
   */
