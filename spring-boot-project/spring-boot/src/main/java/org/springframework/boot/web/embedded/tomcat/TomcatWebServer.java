...
/**
 * {@link WebServer} that can be used to control a Tomcat web server. Usually this class should be
 * created using the {@link TomcatReactiveWebServerFactory} or {@link
 * TomcatServletWebServerFactory}, but not directly.
 *
 * @author Brian Clozel
...
public class TomcatWebServer implements WebServer {
  ...
  public void start() throws WebServerException {
    synchronized (this.monitor) {
      ...
      try {
        ...
        PortInUseException.throwIfPortBindingException(
            ex, () -> this.tomcat.getConnector().getPort());
  ...
  /**
   * Initiates a graceful shutdown of the Tomcat web server. Handling of new requests is prevented
   * and the given {@code callback} is invoked at the end of the attempt. The attempt can be
   * explicitly ended by invoking {@link #stop}.
   *
   * <p>Once shutdown has been initiated Tomcat will reject any new connections. Requests on
   * existing idle connections will also be rejected.
   */
  @Override
