...
/**
 * Log4j2 {@link LogEventPatternConverter} to color output using the {@link AnsiOutput} class. A
 * single option 'styling' can be provided to the converter, or if not specified color styling will
...
public final class ColorConverter extends LogEventPatternConverter {
  ...
  public static ColorConverter newInstance(Configuration config, String[] options) {
    if (options.length < 1) {
      LOGGER.error(
          "Incorrect number of options on style. Expected at least 1, received {}", options.length);
      return null;
    }
    if (options[0] == null) {
