...
/**
 * Extended version of the Logback {@link JoranConfigurator} that adds additional Spring Boot rules.
 *
 * @author Phillip Webb
...
class SpringBootJoranConfigurator extends JoranConfigurator {
  ...
  protected void addModelHandlerAssociations(DefaultProcessor defaultProcessor) {
    defaultProcessor.addHandler(
        ...
        (handlerContext, handlerMic) ->
            new SpringPropertyModelHandler(
                this.context, this.initializationContext.getEnvironment()));
    defaultProcessor.addHandler(
        SpringProfileModel.class,
        (handlerContext, handlerMic) ->
            new SpringProfileModelHandler(
                this.context, this.initializationContext.getEnvironment()));
  ...
  public void addElementSelectorAndActionAssociations(RuleStore ruleStore) {
    ...
    ruleStore.addRule(
        new ElementSelector("configuration/springProperty"), SpringPropertyAction::new);
    ...
    this.modelInterpretationContext.setConfiguratorSupplier(
        () -> {
          SpringBootJoranConfigurator configurator =
              new SpringBootJoranConfigurator(this.initializationContext);
  ...
  public void processModel(Model model) {
    ...
    if (!NativeDetector.inNativeImage() && isAotProcessingInProgress()) {
      getContext()
          .putObject(
              BeanFactoryInitializationAotContribution.class.getName(),
              new LogbackConfigurationAotContribution(
                  model, getModelInterpretationContext(), getContext()));
    }
  }
  ...
  static final class LogbackConfigurationAotContribution
    ...
    private LogbackConfigurationAotContribution(
        Model model, ModelInterpretationContext interpretationContext, Context context) {
  ...
  private static final class ModelWriter {
    ...
    private Set<String> reflectionTypes(Model model, Supplier<Object> parent) {
      ...
      Supplier<Object> componentSupplier =
          SingletonSupplier.ofNullable(() -> instantiate(componentType));
      for (Model submodel : model.getSubModels()) {
    ...
    private Class<?> determineType(Model model, Supplier<Object> parentSupplier) {
      String className =
          (model instanceof ComponentModel componentModel) ? componentModel.getClassName() : null;
      if (className != null) {
      ...
      if (tag != null) {
        className =
            this.modelInterpretationContext
      ...
      if (parent != null) {
        try {
          PropertySetter propertySetter =
              new PropertySetter(this.modelInterpretationContext.getBeanDescriptionCache(), parent);
          Class<?> typeFromPropertySetter =
    ...
    private Class<?> loadComponentType(String componentType) {
      try {
        return ClassUtils.forName(
            this.modelInterpretationContext.subst(componentType), getClass().getClassLoader());
      } catch (Throwable ex) {
    ...
    private void processComponent(Class<?> componentType, Set<String> reflectionTypes) {
      ...
      reflectionTypes.addAll(
          parameterTypesNames(beanDescription.getPropertyNameToAdder().values()));
      reflectionTypes.addAll(
          parameterTypesNames(beanDescription.getPropertyNameToSetter().values()));
  ...
  private static final class RequireNewOrMatchingContentFileHandler
      implements ThrowingConsumer<FileHandler> {
