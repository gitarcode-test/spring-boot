...
class MappingsEndpointServletDocumentationTests extends AbstractEndpointDocumentationTests {
  ...
  @Test
  void mappings() {
    ResponseFieldsSnippet commonResponseFields =
        responseFields(
            ...
            parentIdField());
    List<FieldDescriptor> dispatcherServletFields =
        new ArrayList<>(
            List.of(
                fieldWithPath("*")
                    .description(
                        "Dispatcher servlet mappings, if any, keyed by dispatcher servlet bean"
                            + " name."),
    ...
    List<FieldDescriptor> requestMappingConditions =
        List.of(
            ...
            requestMappingConditionField(".produces.[].negated")
                .description("Whether the media type is negated."));
    List<FieldDescriptor> handlerMethod =
        List.of(
            fieldWithPath("*.[].details.handlerMethod")
                .optional()
                .type(JsonFieldType.OBJECT)
                .description(
    ...
    this.client
        ...
        .consumeWith(
            document(
                ...
                responseFields(
                    beneathPath("contexts.*.mappings.servlets").withSubsectionId("servlets"),
