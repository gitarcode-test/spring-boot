...
class PrometheusSimpleclientExemplarsAutoConfigurationTests {
  ...
  private static final Pattern COUNTER_TRACE_INFO_PATTERN =
      Pattern.compile(
          "^test_observation_seconds_count\\{error=\"none\"} 1.0 #"
      ...
      new ApplicationContextRunner()
          ...
          .withConfiguration(
              AutoConfigurations.of(
                  PrometheusSimpleclientMetricsExportAutoConfiguration.class,
                  PrometheusSimpleclientExemplarsAutoConfiguration.class,
  ...
  void shouldNotSupplyBeansIfPrometheusSupportIsMissing() {
    this.contextRunner
        .withClassLoader(new FilteredClassLoader("io.prometheus.client.exemplars"))
  ...
  void shouldSupplyCustomBeans() {
    this.contextRunner
        ...
        .run(
            (context) ->
                assertThat(context)
                    .hasSingleBean(SpanContextSupplier.class)
    ...
    this.contextRunner.run(
        (context) -> {
          ...
          io.micrometer.prometheus.PrometheusMeterRegistry prometheusMeterRegistry =
              context.getBean(io.micrometer.prometheus.PrometheusMeterRegistry.class);
          String openMetricsOutput =
              prometheusMeterRegistry.scrape(TextFormat.CONTENT_TYPE_OPENMETRICS_100);
          ...
          Optional<TraceInfo> counterTraceInfo =
              openMetricsOutput
                  .lines()
                  .filter(
                      (line) ->
                          line.contains("test_observation_seconds_count")
