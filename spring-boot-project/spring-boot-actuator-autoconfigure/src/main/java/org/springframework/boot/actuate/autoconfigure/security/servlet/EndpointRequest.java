...
/**
 * Factory that can be used to create a {@link RequestMatcher} for actuator endpoint locations.
 *
...
public final class EndpointRequest {
  ...
  /**
   * Returns a matcher that includes all {@link Endpoint actuator endpoints}. It also includes the
   * links endpoint which is present at the base path of the actuator endpoints. The {@link
   * EndpointRequestMatcher#excluding(Class...) excluding} method can be used to further remove
   * specific endpoints if required. For example:
   *
   * <pre class="code">
  ...
  /**
   * Returns a matcher that includes the specified {@link Endpoint actuator endpoints}. For example:
   *
   * <pre class="code">
  ...
  /**
   * Returns a matcher that matches only on the links endpoint. It can be used when security
   * configuration for the links endpoint is different from the other {@link Endpoint actuator
   * endpoints}. The {@link EndpointRequestMatcher#excludingLinks() excludingLinks} method can be
   * used in combination with this to remove the links endpoint from {@link
   * EndpointRequest#toAnyEndpoint() toAnyEndpoint}. For example:
   *
   * <pre class="code">
  ...
  private abstract static class AbstractRequestMatcher
    ...
    protected final boolean matches(
        HttpServletRequest request, Supplier<WebApplicationContext> context) {
    ...
    protected abstract RequestMatcher createDelegate(
        WebApplicationContext context, RequestMatcherFactory requestMatcherFactory);
  ...
  public static final class EndpointRequestMatcher extends AbstractRequestMatcher {
    ...
    private EndpointRequestMatcher(
        List<Object> includes, List<Object> excludes, boolean includeLinks) {
    ...
    @Override
    protected RequestMatcher createDelegate(
        WebApplicationContext context, RequestMatcherFactory requestMatcherFactory) {
      ...
      List<RequestMatcher> delegateMatchers =
          getDelegateMatchers(requestMatcherFactory, matcherProvider, paths);
    ...
    public String toString() {
      return String.format(
          "EndpointRequestMatcher includes=%s, excludes=%s, includeLinks=%s",
    ...
    private EndpointId getEndpointId(Class<?> source) {
      ...
      Assert.state(
          annotation.isPresent(), () -> "Class " + source + " is not annotated with @Endpoint");
