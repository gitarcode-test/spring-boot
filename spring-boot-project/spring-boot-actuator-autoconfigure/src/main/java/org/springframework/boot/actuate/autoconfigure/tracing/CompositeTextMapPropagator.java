...
/**
 * {@link TextMapPropagator} which supports multiple tracing formats. It is able to configure
...
class CompositeTextMapPropagator implements TextMapPropagator {
   ...
   * @param mutuallyExclusiveExtractors the mutually exclusive extractors. They are applied in
   *     order, and as soon as an extractor extracts a context, the other extractors after it are no
   *     longer invoked
   * @param baggagePropagator the baggage propagator to use, or {@code null}
   */
  CompositeTextMapPropagator(
      Collection<TextMapPropagator> injectors,
   ...
   */
  static TextMapPropagator create(
      TracingProperties.Propagation properties, TextMapPropagator baggagePropagator) {
    TextMapPropagatorMapper mapper = new TextMapPropagatorMapper(baggagePropagator != null);
    List<TextMapPropagator> injectors =
        properties.getEffectiveProducedTypes().stream()
            .map(mapper::map)
            .collect(Collectors.toCollection(ArrayList::new));
    if (baggagePropagator != null) {
      injectors.add(baggagePropagator);
    }
    List<TextMapPropagator> extractors =
