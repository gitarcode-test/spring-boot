...
/**
 * {@link brave.propagation.Propagation.Factory Propagation factory} which supports multiple tracing
 * formats. It is able to configure different formats for injecting and for extracting.
 *
...
class CompositePropagationFactory extends Propagation.Factory {
  ...
  CompositePropagationFactory(
      Collection<Factory> injectorFactories, Collection<Factory> extractorFactories) {
  ...
  static CompositePropagationFactory create(
      TracingProperties.Propagation properties,
    ...
    List<Factory> injectors =
        properties.getEffectiveProducedTypes().stream().map(mapper::map).toList();
    List<Factory> extractors =
        properties.getEffectiveConsumedTypes().stream().map(mapper::map).toList();
    return new CompositePropagationFactory(injectors, extractors);
  }

  /**
   * Mapper used to create a {@link brave.propagation.Propagation.Factory Propagation factory} from
   * a {@link PropagationType}.
   */
  ...
  private static class CompositePropagation implements Propagation<String> {
    ...
    CompositePropagation(
        PropagationFactories injectorFactories, PropagationFactories extractorFactories) {
