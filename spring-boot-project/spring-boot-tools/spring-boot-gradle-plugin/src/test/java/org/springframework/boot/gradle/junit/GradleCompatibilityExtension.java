...
/**
 * {@link Extension} that runs {@link TestTemplate templated tests} against multiple versions of
 * Gradle. Test classes using the extension must have a non-private and non-final {@link
 * GradleBuild} field named {@code gradleBuild}.
 *
...
final class GradleCompatibilityExtension implements TestTemplateInvocationContextProvider {
  ...
  public Stream<TestTemplateInvocationContext> provideTestTemplateInvocationContexts(
    ...
    GradleVersion highestVersion =
        GRADLE_VERSIONS.stream()
            .map(GradleVersion::version)
            .collect(Collectors.toCollection(TreeSet::new))
            .last();
    GradleCompatibility gradleCompatibility =
        AnnotationUtils.findAnnotation(context.getRequiredTestClass(), GradleCompatibility.class)
            .get();
  ...
  private static final class GradleVersionTestTemplateInvocationContext
    ...
    public String getDisplayName(int invocationIndex) {
      return "Gradle "
          + this.gradleVersion
          + ((this.configurationCache) ? " --configuration-cache" : "");
    }
