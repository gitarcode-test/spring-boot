...
class WarIntegrationTests extends AbstractArchiveIntegrationTests {
  ...
  void warRepackaging(MavenBuild mavenBuild) {
    mavenBuild
        ...
        .execute(
            (project) ->
                assertThat(jar(new File(project, "target/war-0.0.1.BUILD-SNAPSHOT.war")))
                    ...
                    .manifest(
                        (manifest) ->
                            manifest
                                .hasMainClass("org.springframework.boot.loader.launch.WarLauncher")
  ...
  void jarDependencyWithCustomFinalNameBuiltInSameReactorIsPackagedUsingArtifactIdAndVersion(
    ...
    mavenBuild
        ...
        .execute(
            ((project) ->
                assertThat(jar(new File(project, "war/target/war-0.0.1.BUILD-SNAPSHOT.war")))
  ...
  private String buildWarWithOutputTimestamp(MavenBuild mavenBuild) {
    ...
    mavenBuild
        .project("war-output-timestamp")
        .execute(
            (project) -> {
              File repackaged =
                  new File(project, "target/war-output-timestamp-0.0.1.BUILD-SNAPSHOT.war");
  ...
  @TestTemplate
  void whenWarIsRepackagedWithOutputTimestampConfiguredThenLibrariesAreSorted(
      MavenBuild mavenBuild) {
    mavenBuild
        .project("war-output-timestamp")
        .execute(
            (project) -> {
              ...
              List<String> sortedLibs =
                  Arrays.asList(
                      ...
                      // packaged by Maven
                      "WEB-INF/lib/micrometer-commons",
                      "WEB-INF/lib/micrometer-observation",
                      "WEB-INF/lib/spring-aop",
                      "WEB-INF/lib/spring-beans",
                      "WEB-INF/lib/spring-context",
                      "WEB-INF/lib/spring-core",
                      "WEB-INF/lib/spring-expression",
  ...
  void repackagedWarContainsTheLayersIndexByDefault(MavenBuild mavenBuild) {
    mavenBuild
        ...
        .execute(
            (project) -> {
              File repackaged =
                  new File(project, "war/target/war-layered-0.0.1.BUILD-SNAPSHOT.war");
              assertThat(jar(repackaged))
                  .hasEntryWithNameStartingWith("WEB-INF/classes/")
              ...
              try (JarFile jarFile = new JarFile(repackaged)) {
                ...
                assertThat(layerIndex.get("application"))
                    .contains(
                        "WEB-INF/lib/jar-release-0.0.1.RELEASE.jar",
  ...
  void whenWarIsRepackagedWithTheLayersEnabledAndLayerToolsExcluded(MavenBuild mavenBuild) {
    mavenBuild
        .project("war-layered-no-layer-tools")
        .execute(
            (project) -> {
              File repackaged =
                  new File(project, "war/target/war-layered-0.0.1.BUILD-SNAPSHOT.war");
  ...
  void whenWarIsRepackagedWithToolsExclude(MavenBuild mavenBuild) {
    mavenBuild
        .project("war-no-tools")
        .execute(
            (project) -> {
              File repackaged =
                  new File(project, "war/target/war-no-tools-0.0.1.BUILD-SNAPSHOT.war");
              assertThat(jar(repackaged))
                  .hasEntryWithNameStartingWith("WEB-INF/classes/")
                  .hasEntryWithNameStartingWith("WEB-INF/lib/jar-release")
                  .hasEntryWithNameStartingWith("WEB-INF/lib/jar-snapshot")
            ...
            (project) -> {
              ...
              try (JarFile jarFile = new JarFile(repackaged)) {
                ...
                assertThat(index)
                    .allMatch(
                        (entry) ->
                            entry.startsWith("WEB-INF/lib/")
                                || entry.startsWith("WEB-INF/lib-provided/"));
              }
            });
  }
