...
class MetadataGenerationEnvironment {
  ...
  private static final Set<String> DEPRECATION_EXCLUDES =
      Set.of(
   ...
   * @return the default value or {@code null} if the field does not exist or no default value has
   *     been detected
   */
  ...
  ItemDeprecation resolveItemDeprecation(Element element) {
    AnnotationMirror annotation =
        getAnnotation(element, this.deprecatedConfigurationPropertyAnnotation);
    String reason = null;
  ...
  /**
   * Collect the annotations that are annotated or meta-annotated with the specified {@link
   ...
   */
  List<Element> getElementsAnnotatedOrMetaAnnotatedWith(
  ...
  private boolean collectElementsAnnotatedOrMetaAnnotatedWith(
      TypeElement annotationType, LinkedList<Element> stack) {
  ...
  Map<String, Object> getAnnotationElementValues(AnnotationMirror annotation) {
    ...
    annotation
        ...
        .forEach(
            (name, value) ->
                values.put(name.getSimpleName().toString(), getAnnotationValue(value)));
    return values;
  }
