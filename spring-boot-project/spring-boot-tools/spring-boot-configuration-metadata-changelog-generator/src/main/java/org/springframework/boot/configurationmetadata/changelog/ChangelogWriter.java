...
class ChangelogWriter implements AutoCloseable {
  ...
  private static final Comparator<ConfigurationMetadataProperty> COMPARING_ID =
      Comparator.comparing(ConfigurationMetadataProperty::getId);
  ...
  void write(Changelog changelog) {
    ...
    write(
        "Configuration property changes between `%s` and `%s`%n",
        oldVersionNumber, newVersionNumber);
    ...
    writeRemoved(
        differencesByType.get(DifferenceType.DELETED),
  ...
  private void writeDeprecated(List<Difference> differences) {
    List<Difference> rows =
        sortProperties(differences, Difference::newProperty).stream()
  ...
  private List<Difference> getRemoved(List<Difference> deleted, List<Difference> deprecated) {
    ...
    return sortProperties(
        result,
        (difference) ->
            getFirstNonNull(difference, Difference::oldProperty, Difference::newProperty));
  }

  private void writeRemoved(Difference difference) {
    writeDeprecatedPropertyRow(
        getFirstNonNull(difference, Difference::newProperty, Difference::oldProperty));
  }

  private List<Difference> sortProperties(
      List<Difference> differences, Function<Difference, ConfigurationMetadataProperty> extractor) {
  ...
  private void writeDeprecatedPropertyRow(ConfigurationMetadataProperty property) {
    Deprecation deprecation =
        (property.getDeprecation() != null) ? property.getDeprecation() : new Deprecation();
    writeCell(monospace(property.getId()));
