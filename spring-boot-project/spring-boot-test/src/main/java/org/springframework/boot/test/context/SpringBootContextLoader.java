...
/**
 * A {@link ContextLoader} that can be used to test Spring Boot applications (those that normally
 * startup using {@link SpringApplication}). Although this loader can be used directly, most test
 * will instead want to use it with {@link SpringBootTest @SpringBootTest}.
 *
 * <p>The loader supports both standard {@link MergedContextConfiguration} as well as {@link
 * WebMergedContextConfiguration}. If {@link WebMergedContextConfiguration} is used the context will
 * either use a mock servlet environment, or start the full embedded web server.
 *
 * <p>If {@code @ActiveProfiles} are provided in the test class they will be used to create the
 * application context.
 *
 * @author Dave Syer
 * @author Phillip Webb
...
public class SpringBootContextLoader extends AbstractContextLoader implements AotContextLoader {
  ...
  private ApplicationContext loadContext(
    ...
    if (mainMethod != null) {
      ContextLoaderHook hook =
          new ContextLoaderHook(
              mode, initializer, (application) -> configure(mergedConfig, application));
      return hook.runMain(
  ...
  private void assertHasClassesOrLocations(MergedContextConfiguration mergedConfig) {
    ...
    Assert.state(
        hasClasses || hasLocations,
        () ->
            "No configuration classes or locations found in @SpringApplicationConfiguration. For"
                + " default configuration detection to work you need Spring 4.0.3 or better (found "
                + SpringVersion.getVersion()
                + ").");
  }

  private Method getMainMethod(
      MergedContextConfiguration mergedConfig, UseMainMethod useMainMethod) {
    ...
    Assert.state(
        ...
        () ->
            "UseMainMethod.%s cannot be used with @ContextHierarchy tests"
                .formatted(useMainMethod));
    Class<?> springBootConfiguration =
        Arrays.stream(mergedConfig.getClasses())
            ...
            .orElse(null);
    Assert.state(
        springBootConfiguration != null || useMainMethod == UseMainMethod.WHEN_AVAILABLE,
        "Cannot use main method as no @SpringBootConfiguration-annotated class is available");
    Method mainMethod =
        (springBootConfiguration != null)
            ? ReflectionUtils.findMethod(springBootConfiguration, "main", String[].class)
            : null;
    if (mainMethod == null && KotlinDetector.isKotlinPresent()) {
      try {
        Class<?> kotlinClass =
            ClassUtils.forName(
                springBootConfiguration.getName() + "Kt", springBootConfiguration.getClassLoader());
    ...
    Assert.state(
        mainMethod != null || useMainMethod == UseMainMethod.WHEN_AVAILABLE,
  ...
  /**
   * Return the {@link ApplicationContextFactory} that should be used for the test. By default this
   * method will return a factory that will create an appropriate {@link ApplicationContext} for the
  ...
  protected ApplicationContextFactory getApplicationContextFactory(
    ...
    return (webApplicationType) -> {
      if (webApplicationType != WebApplicationType.NONE
          && !isEmbeddedWebEnvironment(mergedConfig)) {
  ...
  private void prepareEnvironment(
    ...
    TestPropertySourceUtils.addPropertySourcesToEnvironment(
        environment, resourceLoader, mergedConfig.getPropertySourceDescriptors());
    TestPropertySourceUtils.addInlinedPropertiesToEnvironment(
        environment, getInlinedProperties(mergedConfig));
  }

  private void setActiveProfiles(
      ConfigurableEnvironment environment, String[] profiles, boolean applicationEnvironment) {
  ...
  /**
   * Builds new {@link org.springframework.boot.SpringApplication} instance. This method is only
   * called when a {@code main} method isn't being used to create the {@link SpringApplication}.
   *
  ...
  /**
   * Returns the {@link ConfigurableEnvironment} instance that should be applied to {@link
   * SpringApplication} or {@code null} to use the default. You can override this method if you need
  ...
  /**
   * Return the {@link ApplicationContextInitializer initializers} that will be applied to the
   * context. By default this method will adapt {@link ContextCustomizer context customizers}, add
   * {@link SpringApplication#getInitializers() application initializers} and add {@link
   * MergedContextConfiguration#getContextInitializerClasses() initializers specified on the test}.
   *
  ...
  /**
   * Detect the default configuration classes for the supplied test class. By default simply
   * delegates to {@link AnnotationConfigContextLoaderUtils#detectDefaultConfigurationClasses}.
   *
  ...
  private static final class WebConfigurer {
    ...
    private void addMockServletContext(
      ...
      SpringBootMockServletContext servletContext =
          new SpringBootMockServletContext(webMergedConfig.getResourceBasePath());
      initializers.add(
          0,
          new DefensiveWebApplicationContextInitializer(
              new ServletContextApplicationContextInitializer(servletContext, true)));
    }

    /**
     * Decorator for {@link ServletContextApplicationContextInitializer} that prevents a failure
     * when the context type is not as was predicted when the initializer was registered. This can
     * occur when spring.main.web-application-type is set to something other than servlet.
     */
    private static final class DefensiveWebApplicationContextInitializer
      ...
      private DefensiveWebApplicationContextInitializer(
          ServletContextApplicationContextInitializer delegate) {
  ...
  /**
   * Adapts a {@link ContextCustomizer} to a {@link ApplicationContextInitializer} so that it can be
   * triggered through {@link SpringApplication}.
   */
  private static class ContextCustomizerAdapter
    ...
    ContextCustomizerAdapter(
        ContextCustomizer contextCustomizer, MergedContextConfiguration mergedConfig) {
  ...
  private class PrepareEnvironmentListener
    ...
    public void onApplicationEvent(ApplicationEnvironmentPreparedEvent event) {
      prepareEnvironment(
          this.mergedConfig, event.getSpringApplication(), event.getEnvironment(), true);
    }
  }

  /**
   * {@link SpringApplicationHook} used to capture {@link ApplicationContext} instances and to
          ...
          if (ContextLoaderHook.this.mode == Mode.AOT_RUNTIME) {
            application.addInitializers(
                (AotApplicationContextInitializer<?>)
                    ContextLoaderHook.this.initializer::initialize);
          }
        }
