...
/**
 * {@link ContextCustomizer} to allow {@code @Import} annotations to be used directly on test
 * classes.
 *
 * @author Phillip Webb
...
class ImportsContextCustomizer implements ContextCustomizer {
  ...
  private void registerCleanupPostProcessor(
      BeanDefinitionRegistry registry, AnnotatedBeanDefinitionReader reader) {
    BeanDefinition definition =
        registerBean(
            registry,
            reader,
            ImportsCleanupPostProcessor.BEAN_NAME,
  ...
  private void registerImportsConfiguration(
      BeanDefinitionRegistry registry, AnnotatedBeanDefinitionReader reader) {
    BeanDefinition definition =
        registerBean(registry, reader, ImportsConfiguration.BEAN_NAME, ImportsConfiguration.class);
  ...
  private BeanDefinition registerBean(
  ...
  /** {@link Configuration @Configuration} registered to trigger the {@link ImportsSelector}. */
  ...
  /**
   * {@link ImportSelector} that returns the original test class so that direct {@code @Import}
  ...
  static class ImportsSelector implements ImportSelector, BeanFactoryAware {
    ...
    public String[] selectImports(AnnotationMetadata importingClassMetadata) {
      BeanDefinition definition =
          this.beanFactory.getBeanDefinition(ImportsConfiguration.BEAN_NAME);
  ...
  /**
   * {@link BeanDefinitionRegistryPostProcessor} to cleanup temporary configuration added to load
   * imports.
   */
  ...
  /**
   * The key used to ensure correct application context caching. Keys are generated based on
   * <em>all</em> the annotations used with the test that aren't core Java or Kotlin annotations. We
   * must use something broader than just {@link Import @Import} annotations since an
   * {@code @Import} may use an {@link ImportSelector} which could make decisions based on anything
