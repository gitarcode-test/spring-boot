...
class DefaultConnectionPorts implements ConnectionPorts {
  ...
  DefaultConnectionPorts(DockerCliInspectResponse inspectResponse) {
    this.mappings =
        !isHostNetworkMode(inspectResponse)
    ...
    this.mappings.forEach(
        (containerPort, hostPort) -> portMappings.put(containerPort.number(), hostPort));
  ...
  private Map<ContainerPort, Integer> buildMappingsForNetworkSettings(
