...
/**
 * A collection of modules, Maven plugins, and Maven boms that are versioned and released together.
 *
...
public class Library {
  ...
  /**
   * Create a new {@code Library} with the given {@code name}, {@code version}, and {@code groups}.
   *
   * @param name name of the library
   * @param calendarName name of the library as it appears in the Spring Calendar. May be {@code
   *     null} in which case the {@code name} is used.
   ...
   * @param alignsWithBom the coordinates of the bom, if any, that this library should align with
   * @param linkRootName the root name to use when generating link variable or {@code null} to
   *     generate one based on the library {@code name}
  ...
  public static class ProhibitedVersion {
    ...
    public boolean isProhibited(String candidate) {
      ...
      result =
          result
              || (this.range != null
                  && this.range.containsVersion(new DefaultArtifactVersion(candidate)));
  ...
  public static class VersionAlignment {
    ...
    private Map<String, String> resolveAligningDependencies() {
      ...
      for (DependencyResult dependency :
          alignmentConfiguration.getIncoming().getResolutionResult().getAllDependencies()) {
        versions.put(
    ...
    private List<Dependency> getAligningDependencies() {
      if (this.managedBy == null) {
        ...
        return List.of(
            this.project
                .getDependencies()
                .create(this.from + ":" + fromLibrary.getVersion().getVersion()));
      } else {
          ...
          .orElseThrow(
              () ->
                  new IllegalStateException(
                      "Managing library '" + this.managedBy + "' not found."));
    }
    ...
    private List<Dependency> getBomDependencies(Library manager) {
      ...
      return manager.getGroups().stream()
          .flatMap(
              (group) ->
                  group.getBoms().stream()
                      .map(
                          (bom) ->
                              this.project
                                  .getDependencies()
