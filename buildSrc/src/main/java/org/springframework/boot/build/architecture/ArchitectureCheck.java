...
public abstract class ArchitectureCheck extends DefaultTask {
  ...
  public ArchitectureCheck() {
    ...
    getRules()
        .addAll(
            ...
            noClassesShouldCallCollectorsToList(),
        ...
        .addAll(
            getProhibitObjectsRequireNonNull()
                .map(
                    (prohibit) ->
                        prohibit
                            ? noClassesShouldCallObjectsRequireNonNull()
  ...
  private ArchRule
    ...
    return ArchRuleDefinition.methods()
        ...
        .and()
        .haveRawReturnType(
            Predicates.assignableTo("org.springframework.beans.factory.config.BeanPostProcessor"))
  ...
  private ArchCondition<JavaMethod> onlyHaveParametersThatWillNotCauseEagerInitialization() {
    DescribedPredicate<CanBeAnnotated> notAnnotatedWithLazy =
        DescribedPredicate.not(
            CanBeAnnotated.Predicates.annotatedWith("org.springframework.context.annotation.Lazy"));
    DescribedPredicate<JavaClass> notOfASafeType =
        DescribedPredicate.not(
            Predicates.assignableTo("org.springframework.beans.factory.ObjectProvider")
                    ...
                    events.add(
                        SimpleConditionEvent.violated(
                            ...
                            parameter.getDescription()
                                + " will cause eager initialization as it is "
                                + notAnnotatedWithLazy.getDescription()
                                + " and is "
    ...
    return new ArchCondition<>("have no parameters") {
      ...
      public void check(JavaMethod item, ConditionEvents events) {
        ...
        if (!parameters.isEmpty()) {
          events.add(
              SimpleConditionEvent.violated(
                  item, item.getDescription() + " should have no parameters"));
        }
      }
    };
  }
  ...
  private ArchRule noClassesShouldCallStepVerifierStepVerifyComplete() {
    return ArchRuleDefinition.noClasses()
        ...
        .because(
            "it can block indefinitely and expectComplete().verify(Duration) should be used"
                + " instead");
  }
  ...
  private ArchRule noClassesShouldLoadResourcesUsingResourceUtils() {
    return ArchRuleDefinition.noClasses()
        .should()
        .callMethodWhere(
            JavaCall.Predicates.target(With.owner(Predicates.type(ResourceUtils.class)))
                .and(JavaCall.Predicates.target(HasName.Predicates.name("getURL")))
                .and(
                    JavaCall.Predicates.target(
                        HasParameterTypes.Predicates.rawParameterTypes(String.class)))
                .or(
                    JavaCall.Predicates.target(With.owner(Predicates.type(ResourceUtils.class)))
  ...
  private List<ArchRule> noClassesShouldCallObjectsRequireNonNull() {
    return List.of(
        ArchRuleDefinition.noClasses()
            ...
            .because(
                "org.springframework.utils.Assert.notNull(Object, String) should be used instead"),
        ArchRuleDefinition.noClasses()
            .should()
            .callMethod(Objects.class, "requireNonNull", Object.class, Supplier.class)
            .because(
                "org.springframework.utils.Assert.notNull(Object, Supplier) should be used"
                    + " instead"));
  }
