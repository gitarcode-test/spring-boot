...
public abstract class UpgradeDependencies extends DefaultTask {
  ...
  void upgradeDependencies() {
    GitHubRepository repository =
        createGitHub()
            .getRepository(
                this.bom.getUpgrade().getGitHub().getOrganization(),
  ...
  private void applyUpgrades(
    ...
    for (Upgrade upgrade : upgrades) {
      ...
      try {
        ...
        int issueNumber =
            getOrOpenUpgradeIssue(repository, issueLabels, milestone, title, existingUpgradeIssue);
        ...
        if (new ProcessBuilder()
                .command("git", "add", modified.toFile().getAbsolutePath())
                .start()
                .waitFor()
            != 0) {
        ...
        if (new ProcessBuilder().command("git", "commit", "-m", commitMessage).start().waitFor()
            != 0) {
  ...
  private int getOrOpenUpgradeIssue(
      GitHubRepository repository,
      List<String> issueLabels,
      Milestone milestone,
      String title,
      Issue existingUpgradeIssue) {
  ...
  private String getExistingUpgradeIssueMessageDetails(Issue existingUpgradeIssue) {
    ...
    return " (supersedes #"
        + existingUpgradeIssue.getNumber()
        + " "
    ...
    if (!availableLabels.containsAll(issueLabels)) {
      ...
      throw new InvalidUserDataException(
          "Unknown label"
              + suffix
              + ": "
              + StringUtils.collectionToCommaDelimitedString(unknownLabels));
    }
    return issueLabels;
  }
  ...
  private GitHub createGitHub() {
    ...
    try (Reader reader =
        new FileReader(new File(System.getProperty("user.home"), ".bomr.properties"))) {
  ...
  private List<Upgrade> resolveUpgrades(Milestone milestone) {
    List<Upgrade> upgrades =
        new InteractiveUpgradeResolver(
                getServices().get(UserInputHandler.class),
                new MultithreadedLibraryUpdateResolver(
                    getThreads().get(),
                    new StandardLibraryUpdateResolver(
                        new MavenMetadataVersionResolver(getRepositoryUris().get()),
                        determineUpdatePredicates(milestone))))
            .resolveUpgrades(matchingLibraries(), this.bom.getLibraries());
    return upgrades;
  }

  protected List<BiPredicate<Library, DependencyVersion>> determineUpdatePredicates(
      Milestone milestone) {
  ...
  private boolean isNotProhibited(Library library, DependencyVersion candidate) {
    return library.getProhibitedVersions().stream()
        .noneMatch((prohibited) -> prohibited.isProhibited(candidate.toString()));
  }

  private List<Library> matchingLibraries() {
    List<Library> matchingLibraries =
        this.bom.getLibraries().stream().filter(this::eligible).toList();
